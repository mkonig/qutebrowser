...
class HintLabel(QLabel):
    ...
    def __init__(
        self, elem: webelem.AbstractWebElement, context: "HintContext"
    ) -> None:
    ...
    def update_text(self, matched: str, unmatched: str) -> None:
        ...
        if matched:
            ...
            self.setText(
                '<font color="{}">{}</font>{}'.format(match_color, matched, unmatched)
            )
        else:
...
class HintActions:
    ...
    def call_userscript(
        ...
        env = {
            ...
            "QUTE_CURRENT_URL": context.baseurl.toString(
                flags
...
class HintManager(QObject):
        ...
        for string, label in self._context.labels.items():
            try:
                if string.startswith(keystr):
                    ...
                    not self._context.rapid
                    or config.val.hints.hide_unmatched_rapid_hints
                ):
    ...
    def filter_hints(self, filterstr: Optional[str]) -> None:
        ...
        if self._context.hint_mode == "number":
            ...
            if filterstr is not None:
                ...
                self._handle_auto_follow(
                    filterstr=filterstr, visible=self._context.labels
                )
    ...
    @cmdutils.register(
        instance="hintmanager", scope="window", modes=[usertypes.KeyMode.hint]
    )
